import Day8.Operation.{Accumulate, Jump, NoOperation}

object Day8 extends App with Helper {

  def parseInput: Vector[Operation] =
    parseInputLines().map(Operation.parse).toVector
...
//    println(s"Running round ${round}, state = ${state}.")
  ...
  def part1 = println(runProgramRec(parseInput, ProgramState(0, 0)))

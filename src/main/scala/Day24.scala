object Day24 extends App with Helper {
  ...
  def parseInput: Seq[List[Direction]] = {
    parseInputLines().map(Direction.parseMultiple).toVector
  }
  ...
  def part1 = {
    ...
    println(byPoint)
    val blackCells = byPoint.foldLeft(0) {
      case (count, (point, occurences)) =>
...
//    println(s"final black cells = $finalBlackCells  size = ${finalBlackCells.size}")
  }

...
object Day19 extends App with Helper {
  ...
  case class MatchDisjunctions(matchRules: List[MatchRuleIds])
      extends RuleMatcher {
...
//      println(s"matches MatchRuleId $rule for $line is result $result")
  ...
  /**
    ...
    * 36: "b"
    */
  def parseInput(additional: String = ""): Input = {
    val lines = parseInputLines(additional = additional).toList
    val rules = lines

...
object Day20 extends App with Helper {
  ...
  def pickNextFittingImage(
    ...
    images.values
      ...
      .filter {
        _.borders.zip(borderRestrictions).forall {
          case (border, restrictionOpt) =>
...
//        )
  ...
  def part2 = {
    ...
    val actualImage = Image(
      ...
      pixels = combinedImagesWithoutBorders.reverse //reverse row order!
        .foldLeft(Vector.empty[Vector[Boolean]]) { (rows, elem) =>
          val result = elem.foldLeft(Vector.empty[Vector[Boolean]]) {
            (cols, elem2) =>
    ...
    val seaMonsters = allPlacements(actualImage).map(placement =>
      placement -> findSeaMonsters(placement.transformedImage, seaMonster)
    )

    val seaMonsterTrues = seaMonster.map(_.count(identity)).sum

    seaMonsters.foreach {
      case (placement, monstersFound) =>
